...
class LivraisonController extends Controller
{
    ...
    public  function AfficherVPlanningDayAction(){
        ...
        $user=$this->getDoctrine()->getRepository(employee::class)->findOneBy(array('username'=>$username));
    ...
    public  function AfficherLivreurDispoAction(Request $request)
    {
       ...
       $dateL = $request->get('dateL');
        ...
        //$timestamp=$timestamp->setTime($timestamp->format("H"),$timestamp->format("i"),0);
        ...
        $str =rtrim($str,',');
        if($str!='')
        $livraison=$this->getDoctrine()->getManager()->createQuery("select u from BackBundle:employee u where u.roles like '%Livr%' and  u.id not in (".$str.") "  )->getResult();
        else
    $livraison=$this->getDoctrine()->getManager()->createQuery("select u from BackBundle:employee u where u.roles like '%Livr%' "  )->getResult();
...
//$dateL='2020-02-17 3:00:00';
        ...
        $str =rtrim($str,',');
        if($str!='')
            $vehicules=$this->getDoctrine()->getManager()->createQuery("select u from BackBundle:Vehicule u where u.disponibilite=1 and u.id not in (".$str.") "  )->getResult();
        else
            $vehicules=$this->getDoctrine()->getManager()->createQuery("select u from BackBundle:Vehicule u  where u.disponibilite=1 "  )->getResult();
    ...
    public  function AffecterCommandeAction(Request $request){
        ...
        $livplanning->setEmploye($em->getRepository(employee::class)->find($idl));
    ...
    public  function ModifierVehiculePopAction(Request $request){
        ...
        $matricule= $request->get('matriculee');
    ...
    public  function AfficherPlanningAction(){
        ...
        return $this->render('@Back/Livraison/AfficherPlanning.html.twig');
    }
    public  function AfficherPlanningAdminAction(){



        return $this->render('@Back/Livraison/AfficherPlanningAdmin.html.twig');
    }


    public  function afficherCommandesAction(){
        $user = $this->getUser();
        $cmd=$this->getDoctrine()->getRepository(Commande::class)->findBy(["id_u"=>$user]);

        return $this->render('@Back/Livraison/AfficherCommandeParClient.html.twig',array("cmd"=>$cmd));
    ...
    public  function suiviAction(Request $request)
    {
        ...
        $liv=$this->getDoctrine()->getRepository(Livraison_Planning::class)->findOneBy(["commande"=>$cmd]);

        return $this->render('@Front/Livraison/Suivi.html.twig',array("liv"=>$liv));
    ...
    public function statAction(){
        ...
        $em= $this->getDoctrine()->getManager();
        $query = $em->createQuery('SELECT r  ,UPPER(r.statut) as statut ,COUNT(r.id) as num FROM BackBundle:Livraison_Planning r 
 GROUP BY r.statut');
        ...
        for ($i=0 ;$i<count($reservations);$i++){
            $stat=array();
            if($reservations[$i]['statut']==0) {
                array_push($stat,"Non livrée", $reservations[$i]['num'] / $ln);
                $stat = ["Non livrée", $reservations[$i]['num'] * 100 / $ln];
            }else{
                array_push($stat, "Livrée", $reservations[$i]['num'] / $ln);
                $stat = ["Livrée", $reservations[$i]['num'] * 100 / $ln];
            }
            array_push($data,$stat);
        }
        $pieChart->getData()->setArrayToDataTable( $data );
        $pieChart->getOptions()->setTitle('Pourcentages des commandes livrées');
        ...
        return $this->render('@Back/Livraison/chart.html.twig', array('piechart' => $pieChart));
    }
    ...
    public  function PanneAction()
    {
        ...
        return $this->render('@Back/Livraison/Panne.html.twig',array("vehicules"=>$vehicules));

    }

    public  function TomberPanneAction(Request $request)
    {
        $idV = $request->get('id');
        $em=$this->getDoctrine();
        $vehicule=$em->getRepository(Vehicule::class)->find($idV);
    ...
    public  function ReparerAction(Request $request)
    {
        $idV = $request->get('id');
        $em=$this->getDoctrine();
        $vehicule=$em->getRepository(Vehicule::class)->find($idV);
    ...
    public  function AjoutAdresseClientAction(Request $request)
    {

        return $this->render('@Front/Livraison/AjoutAdressClient.html.twig');

    }



    public  function DetailsProduitsAction(Request $request){
        $cmd=$this->getDoctrine()->getRepository(Commande::class)->find(intval($request->get('id')));

        $panier=$this->getDoctrine()->getRepository(Panier::class)->findBy(["id_c"=>$cmd->getId()]);

        $prd=array();
        $i=0;
        foreach ($panier as $p){
            $pp= $this->getDoctrine()->getRepository(Produit::class)->find($p->getIdP()->getId());

            $prd = array(
                'id' => $pp->getId(),
                'libelle' => $pp->getLibelleP(),
                'prix' => $pp->getPrixVente(),
    ...
    public function ReclamationLivraisonAction()
    {
        $em=$this->getDoctrine();
        $pl=$em->getRepository(Livraison_Planning::class)->findBy(array('statut'=>0));
        return $this->render('BackBundle:Livraison:ReclamationLivraison.html.twig',array('pl'=>$pl));

    }

    public function AvertirAction(Request $request,$id)
    {

        $em=$this->getDoctrine();
        $emp=$em->getRepository(employee::class)->find($id);
        $emp->setAvert($emp->getAvert()+1);
        $em->getManager()->flush();
